        ...
        if(TabPilihRawat.getSelectedIndex()==0){
            if(tbRadiologiRalan.getSelectedRow()!= -1){
                ...
                if(NoPermintaan.trim().equals("")){
                    ...
                    //param.put("diagnosa",Klinis); asli
         ...
         if(TabPilihRawat.getSelectedIndex()==0){
            if(tbRadiologiRalan.getSelectedRow()!= -1){
                ...
                if(tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(),0).toString().trim().equals("")){
                    ...
                    param.put("tanggallahir",Sequel.cariIsi("select date_format(tgl_lahir,'%d/%m/%Y') from pasien where no_rkm_medis=?",norm));
                    
                    Valid.MyReport("rptBarcodePermintaanRadiologi.jasper","report","::[ Barcode No.Permintaan Radiologi ]::",param);            
                }
                this.setCursor(Cursor.getDefaultCursor());
            }else{            
                JOptionPane.showMessageDialog(null,"Maaf, silahkan pilih data permintaan...!!!!");
